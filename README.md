...
由于 MVP 版本使用本地文件系统存储上传素材和 SQLite 数据库，需要一台带有足够磁盘空间的云服务器：

| 规模 | CPU | 内存 | 磁盘 | 参考实例 |
|------|-----|------|------|----------|
| 个人使用（~50 次旅行） | 1 核 | 1 GB | 40 GB SSD | 腾讯云轻量 1C1G / AWS Lightsail $5 |
| 小团队（~200 次旅行） | 2 核 | 2 GB | 100 GB SSD | 阿里云 ECS 2C2G / AWS t3.small |
| 中等规模（~1000 次旅行） | 2 核 | 4 GB | 200+ GB SSD | 阿里云 ECS 2C4G / AWS t3.medium |

说明：
- sharp 做图片处理时比较吃内存，建议至少 1 GB
- 磁盘空间主要取决于素材量，按每次旅行平均 200 张照片（每张 5MB）估算
- 建议选择 SSD 磁盘，缩略图生成和哈希计算有大量随机读写
- 后续素材量增大可考虑迁移到对象存储（S3 / OSS）
