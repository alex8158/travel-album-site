...
// GET /api/users/me/trips — Return current user's all trips (public + private), ordered by created_at DESC
router.get('/me/trips', (req: Request, res: Response) => {
  const db = getDb();
...
// GET /api/users/me/trips/:id/gallery — Return all media for user's own trip
router.get('/me/trips/:id/gallery', (req: Request, res: Response) => {
  const db = getDb();
