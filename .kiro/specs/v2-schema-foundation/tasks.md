...
- [ ] 2. 创建 Media Versions 路由
  - [x] 2.1 创建 `server/src/routes/mediaVersions.ts`，实现 POST/GET/GET:id/DELETE 四个端点
...
- [ ] 3. 创建 Media Analysis 路由
  - [x] 3.1 创建 `server/src/routes/mediaAnalysis.ts`，实现 POST/GET/PUT 三个端点
    - POST 创建分析记录
...
- [ ] 4. 创建 Duplicate Group Items 路由
  - [x] 4.1 创建 `server/src/routes/duplicateGroupItems.ts`，实现 POST/GET/PUT/DELETE 四个端点
...
- [ ] 5. 创建 AI Invocations 路由
  - [x] 5.1 创建 `server/src/routes/aiInvocations.ts`，实现 POST/GET/PUT/GET:summary 四个端点
    - POST 创建调用记录
...
- [x] 7. 创建数据迁移服务 analysisMigration.ts
...
- [x] 9. Checkpoint - 确认编译通过
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 10. 编写单元测试
  - [x] 10.1 创建 `server/src/database.test.ts` 中追加新表 schema 验证测试
    - 验证 4 张新表和所有索引存在
    - 验证外键约束拒绝无效引用
    - 验证 duplicate_group_items 的唯一约束
    - _Requirements: 1.1, 2.1, 3.1, 3.3, 4.1_
  - [x] 10.2 创建 `server/src/routes/mediaVersions.test.ts`
    - 测试 CRUD happy path 和 version_type 校验
    - _Requirements: 5.1, 5.2, 5.3, 5.4, 5.5_
  - [x] 10.3 创建 `server/src/routes/mediaAnalysis.test.ts`
    - 测试 CRUD happy path 和 history 查询
    - _Requirements: 6.1, 6.2, 6.3, 6.4_
  - [x] 10.4 创建 `server/src/routes/duplicateGroupItems.test.ts`
    - 测试 CRUD happy path 和 409 冲突
    - _Requirements: 7.1, 7.2, 7.3, 7.4, 7.5_
  - [x] 10.5 创建 `server/src/routes/aiInvocations.test.ts`
    - 测试 CRUD happy path、过滤、聚合统计
    - _Requirements: 8.1, 8.2, 8.3, 8.4_
  - [x] 10.6 创建 `server/src/services/analysisMigration.test.ts`
